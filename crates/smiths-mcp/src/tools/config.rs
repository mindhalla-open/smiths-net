//! `get_config` + `put_config`: live config read / patch layered on
//! the `ConfigReloader` substrate. `get_config` reads, `put_config`
//! mutates through the same `apply` path SIGHUP drives.
    ...
    async fn call(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        ...
        let receipt = reloader
            ...
            .map_err(|e| match e {
                ...
                smiths_core::ApplyError::Invalid(e) => ToolError::InvalidArguments(e.to_string()),
                smiths_core::ApplyError::Pending => ToolError::InvalidArguments(e.to_string()),
            })?;
...
mod tests {
    ...
    fn test_ctx() -> (ToolContext, TestEngine) {
        let engine = TestEngine::new();
        (engine.ctx.clone(), engine)
    }

    fn test_ctx_with_reloader() -> (ToolContext, Arc<ConfigReloader>, TestEngine) {
