//! Fixed-offset binary wire format.
//!
//! Hand-rolled flat layouts for [`RtpFrame`] and [`Envelope`]: every
//! fixed-width field sits at a known little-endian offset and the
//! variable-length regions follow in a fixed order, so a reader can
//! borrow any field straight out of the buffer. No tags, no varints,
//! no schema compiler, no `unsafe`. This is **not** `FlatBuffers` —
//! there are no vtables and no compatibility with the `FlatBuffers`
//! runtime; the module name says what it is.
//!
...
//!
//! The `fixed_frame_bench` test in `tests/` is an explicit,
//! `#[ignore]`d benchmark that measures the round-trip speedup on a
//! 160-byte RTP frame.
//!
//! ## Layout — `RtpFrame`
//!
...
/// Fixed-offset [`WireFormat`]. Stateless; cheap to `Clone`.
#[derive(Clone, Copy, Debug, Default)]
pub struct FixedFrameWireFormat;
...
fn encode_envelope_fixed(envelope: &Envelope) -> Vec<u8> {
...
fn decode_envelope_fixed(bytes: &[u8]) -> Result<Envelope, WireFormatError> {
...
mod tests {
    ...
    fn rtp_frame_magic_mismatch_is_error() {
        let mut bytes = FixedFrameWireFormat.encode_rtp_frame(&sample_frame());
        bytes[0] = b'X';
        let err = FixedFrameWireFormat.decode_rtp_frame(&bytes).unwrap_err();
        assert!(matches!(err, WireFormatError::Malformed(_)));
    }
