//! Wire-format throughput benchmark — an explicit, self-timed loop,
//! not a regression test.
//!
//! Compares prost against the fixed-offset layout on the RTP hot
//! path and asserts the fixed-frame encode+decode round trip is at
//! least 2× faster than prost's on a 160-byte PCMU-shaped frame,
//! which is the layout's design goal.
//!
//! Marked `#[ignore]` so CI doesn't pay for (or flake on) timing;
//! run it by hand with
//! `cargo test -p smiths-proto --test fixed_frame_bench -- --ignored --nocapture`.
...
use smiths_proto::{FixedFrameWireFormat, ProtoWireFormat, RtpFrame, WireFormat, WireFormatKind};
...
#[test]
#[ignore = "benchmark, not a test: run with --ignored --nocapture to see numbers"]
fn fixed_frame_round_trip_is_at_least_2x_prost() {
    let frame = sample_frame();
    let proto = ProtoWireFormat;
    let flat = FixedFrameWireFormat;

    assert_eq!(proto.kind(), WireFormatKind::Proto);
    assert_eq!(flat.kind(), WireFormatKind::FixedFrame);
    ...
    println!(
        "[bench] rtp frame n={n} iters\n\
         ...
         [bench]   fixed-frame : {flat_ns} ns/iter  ({flat_bytes} bytes/msg)\n\
    ...
    assert!(
        speedup >= 2.0,
        "design goal is a ≥2× speedup; got {speedup:.2}×. \
