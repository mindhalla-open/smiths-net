...
/// Trait generated by `#[derive(Reloadable)]` — diffs a candidate
/// config tree against the prior one, pushing every changed
/// reloadable / restart-required field into `report`.
///
/// Callers start recursion at the root with `path_prefix = ""`;
/// nested derived impls extend the prefix automatically. Composite
/// semantics (e.g. `"sip.rate_limit"` treating its two fields as
/// one unit) are expressed with `#[reloadable(path = "…")]` on the
/// parent field — the child struct doesn't need to derive
/// `Reloadable`, only `PartialEq`.
///
/// The hand-written `Config::apply_report` delegates to this
/// trait; adding a new config field in the tree is now "annotate
/// the field" instead of "edit the hardcoded diff body + hope
/// code review catches the drift".
pub trait Reloadable {
    /// Push every changed reloadable / restart-required field
    /// into `report`. `path_prefix` is the dotted path accumulated
    /// by outer derived impls (empty at the top level).
    fn diff_into(&self, new: &Self, report: &mut ApplyReport, path_prefix: &str);
}

// Re-export the derive macro. Rust's namespace split (macro vs
// type) means the derive shares the `Reloadable` name with the
// trait without ambiguity — users write `use smiths_core::Reloadable`
// once and get both, same trick `serde::Serialize` uses.
pub use smiths_config_macros::Reloadable;
...
impl Config {
    ...
    /// `new` (candidate).
    ///
    /// Delegates to the `#[derive(Reloadable)]`-generated
    /// [`Reloadable::diff_into`] walk so the reloadable /
    /// restart-required field list lives next to the fields
    /// themselves and can't drift against the real struct.
    ///
    /// Conservative default: any field without a
    /// `#[reloadable]` / `#[restart_required]` / `#[nested]`
    /// annotation is treated as out-of-scope for hot reload —
    /// changing it neither trips restart nor bumps the
    /// reloaded list. Review the annotations when adding new
    /// config fields.
    #[must_use]
...
mod tests {
    ...
    type Mutator = Box<dyn FnOnce(&mut Config)>;

    fn diff_case(mutator: Mutator) -> ApplyReport {
        let base = Config::default();
        let mut candidate = base.clone();
        mutator(&mut candidate);
        let mut report = ApplyReport::default();
        base.diff_into(&candidate, &mut report, "");
        report
    }

    /// Byte-identity: every reloadable path the derive walk
    /// emits matches the legacy hand-written `apply_report`. If
    /// a mark drifts or a path gets renamed we catch it here
    /// before a downstream adapter starts receiving an
    /// unrecognised field name.
    #[test]
    fn diff_into_emits_expected_reloadable_paths() {
        use crate::config::{SipRateLimit, TranscodeConfig};
        let cases: Vec<(&str, Mutator)> = vec![
            (
                "observability.log_level",
                Box::new(|c| c.observability.log_level = "trace".into()),
            ),
            (
                "sip.rate_limit",
                Box::new(|c| {
                    c.sip.rate_limit = SipRateLimit {
                        per_sec: 42,
                        burst: 42,
                    }
                }),
            ...
            (
                "ai.openai_api_key",
                Box::new(|c| c.ai.openai_api_key = Some("rot".into())),
            ),
            (
                "ai.anthropic_api_key",
                Box::new(|c| c.ai.anthropic_api_key = Some("rot".into())),
            ),
            (
                "media.prompts.capacity",
                Box::new(|c| c.media.prompts.capacity = 999),
            ),
            (
                "media.transcode",
                Box::new(|c| {
                    c.media.transcode = TranscodeConfig {
                        max_concurrent_calls: 99,
                        cpu_budget_ms_per_call: 99,
                    }
                }),
            ),
        ];
        for (expected, mutator) in cases {
            ...
            assert_eq!(
                ...
                "reloaded path mismatch for {expected}"
            );
            assert!(
                report.restart_required.is_empty(),
                "{expected} shouldn't trip restart_required"
            );
        }
    }

    /// Same byte-identity contract for the restart-required
    /// side: every group label coalesces multiple fields into
    /// one report entry and matches the legacy string verbatim.
    #[test]
    fn diff_into_emits_expected_restart_required_groups() {
        use crate::config::{AuthBackend, LogFormat, SipTransport, StorageBackend};
        use std::net::{Ipv4Addr, SocketAddr};

        let cases: Vec<(&str, Mutator)> = vec![
            (
                "sip bind / transports / tls paths",
                Box::new(|c| {
                    c.sip.transports = vec![SipTransport::Tcp];
                    c.sip.tls_cert_path = Some("/tmp/cert.pem".into());
                }),
            ),
            (
                "mcp binds",
                Box::new(|c| {
                    c.mcp.enabled_http = !c.mcp.enabled_http;
                    c.mcp.http_bind = SocketAddr::new(Ipv4Addr::LOCALHOST.into(), 9999);
                }),
            ),
            ("a2a bind", Box::new(|c| c.a2a.enabled = !c.a2a.enabled)),
            (
                "observability bind / log format",
                Box::new(|c| c.observability.log_format = LogFormat::Pretty),
            ),
            ("plugins.dir", Box::new(|c| c.plugins.dir = "other".into())),
            (
                "auth backend",
                Box::new(|c| c.auth.backend = AuthBackend::Sqlite),
            ),
            (
                "storage backend",
        ...
        for (expected, mutator) in cases {
            ...
            assert_eq!(
                ...
                vec![expected.to_string()],
                "restart_required label mismatch for {expected}"
            );
            assert!(
                report.reloaded.is_empty(),
                "{expected} shouldn't trip reloaded"
            );
        }
    }
}
